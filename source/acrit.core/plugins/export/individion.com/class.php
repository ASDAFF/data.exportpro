...
class IndividionCom extends Plugin
{
	...
	public function stepExport($intProfileID, $arData)
	{
		...
		/*
		  # SubStep8 [<promos>]
		  if ($bGiftsExport && !isset($arSession['XML_PROMOS_WROTE']))
		  {
	...
	protected function stepExport_writeXmlPromos($intProfileID, $arData)
	{
		...
		#
		if (empty($arGiftIDsExported) && empty($arGiftIDsInOffers))
		{
			Log::getInstance($this->strModuleId)->add(static::getMessage('GIFTS_ARE_NOT_FOUND'), $intProfileID, true);
			return Exporter::RESULT_SUCCESS;
		}
		$strXml = "\t\t" . '<promos>' . "\r\n";
		file_put_contents($strFile, $strXml, FILE_APPEND);
		# 1. find all gifts with its element
		$intOffset = 0;
		$intLimit = 5000;
		while (true)
		{
