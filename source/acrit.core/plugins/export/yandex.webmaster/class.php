<?
/**
 * Acrit Core: Yandex.Webmaster base plugin
...
class YandexWebmaster extends Plugin {
	
	CONST DATE_UPDATED = '2018-07-31';
			...
			if(file_put_contents($strTmpFile, $strFileContent)){
				...
				if(strlen($strCategories)){
					...
					if(file_put_contents($strFileName, $strCategories)){
						...
						Log::getInstance($this->strModuleId)->add(static::getMessage('ERROR_SAVING_CATEGORIES_TMP', array('#FILE#' => $strFileName)), $intProfileID);
					}
				}
				else{
	...
	public function getFields($intProfileID, $intIBlockID, $bAdmin=false){
		...
		$this->addUtmFields($arResult, 901, 'yandex.ru');
		$arResult[] = new Field(array(
			'CODE' => 'PRICE',
			'DISPLAY_CODE' => 'price',
