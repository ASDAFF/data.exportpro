...
class YandexWebmasterAudiobooks extends YandexWebmaster {
	...
	public static function getCode() {
		return parent::getCode().'_AUDIOBOOKS';
	}
	...
	public function getFields($intProfileID, $intIBlockID, $bAdmin=false){
		...
		$arResult[] = new Field(array(
			'CODE' => 'PERFORMED_BY',
			'DISPLAY_CODE' => 'performed_by',
			'NAME' => static::getMessage('FIELD_PERFORMED_BY_NAME'),
			'SORT' => 200,
			'DESCRIPTION' => static::getMessage('FIELD_PERFORMED_BY_DESC'),
			'REQUIRED' => false,
			'MULTIPLE' => false,
			'DEFAULT_VALUE' => array(
				array(
					'TYPE' => 'FIELD',
					'VALUE' => 'PROPERTY_PERFORMED_BY',
				),
			),
		));
		$arResult[] = new Field(array(
			'CODE' => 'PERFORMANCE_TYPE',
			'DISPLAY_CODE' => 'performance_type',
			'NAME' => static::getMessage('FIELD_PERFORMANCE_TYPE_NAME'),
			'SORT' => 210,
			'DESCRIPTION' => static::getMessage('FIELD_PERFORMANCE_TYPE_DESC'),
			'REQUIRED' => false,
			'MULTIPLE' => false,
			'DEFAULT_VALUE' => array(
				array(
					'TYPE' => 'FIELD',
					'VALUE' => 'PROPERTY_PERFORMANCE_TYPE',
				),
			),
		));
		$arResult[] = new Field(array(
			...
			'NAME' => static::getMessage('FIELD_STORAGE_NAME'),
			'SORT' => 220,
			'DESCRIPTION' => static::getMessage('FIELD_STORAGE_DESC'),
			'REQUIRED' => false,
			'MULTIPLE' => false,
			'DEFAULT_VALUE' => array(
				array(
					'TYPE' => 'FIELD',
					'VALUE' => 'PROPERTY_STORAGE',
				),
			),
		));
		$arResult[] = new Field(array(
			'CODE' => 'FORMAT',
			'DISPLAY_CODE' => 'format',
			'NAME' => static::getMessage('FIELD_FORMAT_NAME'),
			'SORT' => 230,
			'DESCRIPTION' => static::getMessage('FIELD_FORMAT_DESC'),
			'REQUIRED' => false,
			'MULTIPLE' => false,
			'DEFAULT_VALUE' => array(
				array(
		...
		$arResult[] = new Field(array(
			...
			'DISPLAY_CODE' => 'recording_length',
			'NAME' => static::getMessage('FIELD_RECORDING_LENGTH_NAME'),
			'SORT' => 240,
			'DESCRIPTION' => static::getMessage('FIELD_RECORDING_LENGTH_DESC'),
			'REQUIRED' => false,
			'MULTIPLE' => false,
			'DEFAULT_VALUE' => array(
				array(
					'TYPE' => 'FIELD',
					'VALUE' => 'PROPERTY_RECORDING_LENGTH',
				),
			),
		));
		#
	...
	public function processElement($arProfile, $intIBlockID, $arElement, $arFields){
		...
		if(!Helper::isEmpty($arFields['PERFORMED_BY']))
			$arXmlTags['performed_by'] = Xml::addTag($arFields['PERFORMED_BY']);
		if(!Helper::isEmpty($arFields['PERFORMANCE_TYPE']))
			$arXmlTags['performance_type'] = Xml::addTag($arFields['PERFORMANCE_TYPE']);
		if(!Helper::isEmpty($arFields['STORAGE']))
			$arXmlTags['storage'] = Xml::addTag($arFields['STORAGE']);
		if(!Helper::isEmpty($arFields['FORMAT']))
			$arXmlTags['format'] = Xml::addTag($arFields['FORMAT']);
		...
		if(!Helper::isEmpty($arFields['RECORDING_LENGTH']))
			$arXmlTags['recording_length'] = Xml::addTag($arFields['RECORDING_LENGTH']);
		...
		$arXml = array(
			'offer' => array(
				'@' => $this->getXmlAttr($intProfileID, $arFields, 'audiobook'),
				'#' => $arXmlTags,
			),
		);
