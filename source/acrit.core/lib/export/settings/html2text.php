...
class SettingsHtml2text extends SettingsBase {
	...
	public static function showSettings($strFieldCode, $obField, $arParams){
		...
		<span id="<?=static::getInputID();?>_mode_span">
			<?
				$arOptions = array(
					'simple' =>  static::getMessage('TYPE_SIMPLE'),
					'bitrix' =>  static::getMessage('TYPE_BITRIX'),
					'html2text' =>  static::getMessage('TYPE_HTML2TEXT'),
					'custom' =>  static::getMessage('TYPE_CUSTOM'),
				);
				$arOptions = array(
					'REFERENCE' => array_values($arOptions),
					'REFERENCE_ID' => array_keys($arOptions),
				);
				print SelectBoxFromArray(static::getCode().'_mode', $arOptions, 
					$arParams[static::getCode().'_mode'], '', 'id="'.static::getInputID().'_mode"');
			?>
		</span>
		<?=Helper::ShowHint(static::getHint());?>
		<script>
		$('#<?=static::getInputID();?>').bind('change', function(){
			var subSettings = $('#<?=static::getInputID();?>_mode_span');
			if($(this).is(':checked')){
				subSettings.show();
			}
			else{
	...
	public static function process(&$mValue, $arParams, $obField=null){
		if($arParams[static::getCode()] == 'Y'){
			...
			static::processMultipleValue($mValue, $arParams, $obField, function(&$strValue, $arParams, $obField){
				$strMode = $arParams[static::getCode().'_mode'];
				switch($strMode){
					case 'bitrix':
						$strValue = \HTMLToTxt($strValue, '', array(), false);
						break;
					case 'html2text':
						if(Helper::isUtf()){
							$obHtml = new Html2Text_utf8($strValue);
						}
						else{
							$obHtml = new Html2Text_cp1251($strValue);
						}
						$strValue = $obHtml->getText();
						unset($obHtml);
						break;
					case 'custom':
						$mValueOriginal = $strValue;
						if(is_object($obField) && strlen($obField->getModuleId())){
							foreach (EventManager::getInstance()->findEventHandlers($obField->getModuleId(), 'OnCustomHtmlToText') as $arHandler) {
								\ExecuteModuleEventEx($arHandler, array(&$strValue, $arParams, $obField));
							}
						}
						if(is_null($strValue)){
							$strValue = static::processSimple($mValueOriginal);
						}
						break;
					default:
						$strValue = static::processSimple($strValue);
						break;
				}
			});
			#
	...
	protected static function processSimple($mValue){
		$mValue = preg_replace_callback('#(<li[^>]*>(.*?)</li>)#is', function($arMatches){
			return ' - '.trim(strip_tags($arMatches[2]));
		}, $mValue);
