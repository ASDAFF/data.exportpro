...
function CheckFields(){
    ...
    if( intval( $ID ) > 0 ){
        $export = new CDataExportproExport( $ID );
    }
    ...
    foreach( $requiredFields as $field ){
        if( !$PROFILE[$field] ){
            $APPLICATION->ThrowException( GetMessage( "DATA_EXPORTPRO_REQUIRED_FIELD_FAIL" ).'"'.GetMessage( "DATA_EXPORTPRO_STEP1_".$field ).'"' );
            return false;
        }
    }
    ...
    foreach( $PROFILE["XMLDATA"] as $id => $field ){
        if( $field["REQUIRED"] == "Y" ){
            if( ( $field["TYPE"] == "field" && !$field["VALUE"] ) || ( $field["TYPE"] == "const" && !$field["CONTVALUE_TRUE"] ) || $field["TYPE"] == "none" ){
                $APPLICATION->ThrowException( GetMessage( "DATA_EXPORTPRO_REQUIRED_FIELD_FAIL", array( "#CODE#" => $field["CODE"], "#NAME#" => $field["NAME"] ) ) );
                return false;
            }
        }
    }
...
if( $_SERVER["REQUEST_METHOD"] == "POST" && ( !empty( $save ) || !empty( $apply ) ) && check_bitrix_sessid() ){
    if( ( $fieldsCheck = CheckFields() ) ){
        ...
        if( $save ){
            LocalRedirect( "data_exportpro_list.php" );
            die();
        }
        else{
            ...
            if( !isset( $arQuery["ID"] ) || empty( $arQuery["ID"] ) ){
                $arQuery["ID"] = $ID;
                LocalRedirect( "data_exportpro_edit.php?".http_build_query( $arQuery ) );
                die();
            }
        }
    }
...
if( !isset( $_REQUEST["ajax"] ) && !isset( $_REQUEST["ib"] ) && !isset( $_REQUEST["ajax_start"] ) && !isset( $_REQUEST["ajax_count"] ) && !isset( $_POST["auth"] ) ){
    ...
    $APPLICATION->AddHeadScript( "/bitrix/js/data.exportpro/main.js" );
    ...
    $message = null;
    $bVarsFromForm = false;
    $profileTitle = GetMessage( "DATA_EXPORTPRO_EDITPROFILE" ).": #".$arProfile["ID"]." ".$arProfile["NAME"];
    $APPLICATION->SetTitle( ( $ID > 0 ? $profileTitle : GetMessage( "DATA_EXPORTPRO_ADDPROFILE" ) ) );
    if( $copy && $ID > 0 ){
        unset( $arProfile["ID"] );       
    ...
    $aMenu = array(
        array(
            "TEXT" => GetMessage( "DATA_EXPORTPRO_LIST" ),
            "TITLE" => GetMessage( "DATA_EXPORTPRO_LIST" ),
            "LINK" => "data_exportpro_list.php?lang=".LANG,
            "ICON" => "btn_list",
        ),
        array(
            "TEXT" => GetMessage( "DATA_EXPORTPRO_INSTRUCTION" ),
            "TITLE" => GetMessage( "DATA_EXPORTPRO_INSTRUCTION" ),
    ...
    if( $ID > 0 ){
        ...
        $aMenu[] = array(
            "TEXT" => GetMessage( "DATA_EXPORTPRO_ADD" ),
            "TITLE" => GetMessage( "DATA_EXPORTPRO_ADD" ),
            "LINK" => "data_exportpro_edit.php?lang=".LANG,
            "ICON" => "btn_new",
        );
        $aMenu[] = array(
            "TEXT" => GetMessage( "DATA_EXPORTPRO_COPY" ),
            "TITLE" => GetMessage( "DATA_EXPORTPRO_COPY" ),
            "LINK" => "data_exportpro_edit.php?copy=$ID&ID=$ID&lang=".LANG,
            "ICON" => "btn_copy",
        );
        $aMenu[] = array(
            "TEXT" => GetMessage( "DATA_EXPORTPRO_DEL" ),
            "TITLE" => GetMessage( "DATA_EXPORTPRO_DEL" ),
            "LINK" => "javascript:if(confirm('".GetMessage( "parser_mnu_del_conf" )."'))window.location='data_exportpro_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
            "ICON" => "btn_delete",
        );
        $aMenu[] = array( "SEPARATOR" => "Y" );
        $aMenu[] = array(
            "TEXT" => GetMessage( "DATA_EXPORTPRO_RUN" ),
            "TITLE" => GetMessage( "DATA_EXPORTPRO_RUN" ),
            "LINK" => "/bitrix/tools/data.exportpro/data_exportpro.php?ID=".$ID,
    ...
    $aTabs = array(
        array(
            "DIV" => "step3",
            "TAB" => GetMessage( "DATA_EXPORTPRO_TAB3" ),
            "ICON" => "main_user_edit",
            "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB3" )
        ),
    );

    $aTabs[] = array( "DIV" => "step16", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB16" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB16" ) );
    $aTabs[] = array( "DIV" => "step17", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB17" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB17" ) );
    $aTabs[] = array( "DIV" => "step18", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB18" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB18" ) );
    $aTabs[] = array( "DIV" => "step1", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB1" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB1" ) );
    $aTabs[] = array( "DIV" => "step2", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB2" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB2" ) );
    $aTabs[] = array( "DIV" => "step4", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB4" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB4" ) );
    $aTabs[] = array( "DIV" => "step5", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB5" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB5" ) );
    $aTabs[] = array( "DIV" => "step19", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB19" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB19" ) );
    $aTabs[] = array( "DIV" => "step21", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB21" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB21" ) );

    if( CModule::IncludeModule( "catalog" ) ){
        $aTabs[] = array( "DIV" => "step6", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB6" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB6" ) );
        $aTabs[] = array( "DIV" => "step7", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB7" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB7" ) );
        $aTabs[] = array( "DIV" => "step8", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB8" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB8" ) );
        $aTabs[] = array( "DIV" => "step12", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB12" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB12" ) );
        $aTabs[] = array( "DIV" => "step13", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB13" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB13" ) );
    }

    $aTabs[] = array( "DIV" => "step10", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB10" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB10" ) );
    $aTabs[] = array( "DIV" => "step9", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB9" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB9" ) );
    $aTabs[] = array( "DIV" => "step11", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB11" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB11" ) );
    $aTabs[] = array( "DIV" => "step14", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB14" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB14" ) );
    $aTabs[] = array( "DIV" => "step15", "TAB" => GetMessage( "DATA_EXPORTPRO_TAB15" ), "ICON" => "main_user_edit", "TITLE" => GetMessage( "DATA_EXPORTPRO_TAB15" ) );
    ...
    ?>
    <div class="adm-info-message"<?if( $bDemo ){?> style="float: left;"<?}?>><?=GetMessage( "DATA_EXPORTPRO_EXPORT_CHANGE_PROFILE_INFO" );?></div>

    <?if( $bDemo ){?>
        <div class="adm-info-message" style="float: right; padding-left: 20px;">
            <div style="padding-left: 20px; float: right;">
                <a href="<?=$byLicenceUrl?>" target="_blank" class="adm-btn adm-btn-save"><?=GetMessage( "DATA_EXPORTPRO_EXPORT_BUY_LICENCE_INFO" )?></a>
            </div>
            <div style="float: left; padding-top: 5px;"><?=GetMessage( "DATA_EXPORTPRO_EXPORT_DEMO_PERIOD_INFO" )."<b>".$timeDemoOff."</b>";?></div>
    ...
    <?// check session id?>
        ...
        $tabControl->Buttons(
            array(
                ...
                "back_url"=>"data_exportpro_list.php?lang=".LANG,
            )
        );
        ...
        function ShowMarketCategoryList( categoryId, listContainer ){
            ...
            if( typeof MarketCategoryObject == 'undefined' ){
                MarketCategoryObject = new BX.PopupWindow( 'my_answer', null, {
                    ...
                    titleBar: { content: BX.create( 'span', { html: '<?=GetMessage( "DATA_EXPORTPRO_MARKET_CATEGORY_POPUP_TITLE" );?>', 'props': { 'className': 'access-title-bar' } } ) },
                    zIndex: 0,
                    offsetLeft: 0,
                    offsetTop: 0,
                    ...
                    buttons: [
						...
						//	  text: '<?=GetMessage( "DATA_EXPORTPRO_MARKET_CATEGORY_BUTTON_SEND" );?>',
                        ...
                        //   }),
                        //   new BX.PopupWindowButton({
						//	  text: '<?=GetMessage( "DATA_EXPORTPRO_MARKET_CATEGORY_BUTTON_CLOSE" );?>',
        ...
        function ShowPropertyList( obj ){
            ...
            if( typeof PropertyListObject == 'undefined' ){
                PropertyListObject = new BX.PopupWindow( 'property_list_answer', null, {
                    ...
                    closeIcon: { right: '20px', top: '10px' },
                    titleBar: { content: BX.create( 'span', { html: '<?=GetMessage( "DATA_EXPORTPRO_VARIANT_CATEGORY_POPUP_TITLE" );?>', 'props': { 'className': 'access-title-bar' } } ) },
                    zIndex: 0,
                    offsetLeft: 0,
                    offsetTop: 0,
                    draggable: { restrict: false },
