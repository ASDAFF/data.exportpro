...
?>
<tr class="heading">
    <td colspan="2" valign="top"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_RUN" )?></td>
</tr>
<?if( $arProfile["SETUP"]["FILE_TYPE"] == "csv" ){?>
    <tr id="tr_csv_info">
        <td colspan="2">
            <?=BeginNote();?>
            <?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_CSV_INFO" );?>
            <?=EndNote();?>
        </td>
    </tr>
<?}?>
<tr>
    <td width="40%">
        <span id="hint_PROFILE[SETUP][EXPORT_STEP]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][EXPORT_STEP]' ), '<?=GetMessage( "KIT_EXPORTPRO_EXPORTP_STEP_HELP" );?>' );</script>
        <label for="PROFILE[SETUP][EXPORT_STEP]"><?=GetMessage( "KIT_EXPORTPRO_EXPORTP_STEP" )?></label>
    </td>
    <td width="60%" id="export_step_block">
        <?if( intval( $arProfile["ID"] ) > 0 ){?>
            ...
            <div style="margin-top: -3px;">
                <a class="adm-btn adm-btn-save" onclick="CalcExportStep( <?=$arProfile["ID"];?> )"><?=GetMessage( "KIT_EXPORTPRO_EXPORTP_STEP_CALC" )?></a>
            </div>
        ...
        <table cellpadding="0" cellspacing="0" width="100%">
            <tbody>
                <tr>
                    <td colspan="2" align="center">
                        <?=BeginNote();?>
                        <?=GetMessage( "KIT_EXPORTPRO_RUN_EXPORT_FILE_DESCRIPTION" );?>
                        <?=EndNote();?>
                    </td>
                </tr>
                <tr id="check_compress_block">
                    <?if( ( $arProfile["TYPE"] != "advantshop" ) ){?>
                        <td width="40%" class="adm-detail-content-cell-l">
                            <span id="hint_PROFILE[USE_COMPRESS]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[USE_COMPRESS]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_USE_COMPRESS_HELP" );?>' );</script>
                            <label for="PROFILE[USE_COMPRESS]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_USE_COMPRESS" );?></label>
                        </td>
                ...
                <tr>
                    <td width="40%" class="adm-detail-content-cell-l">
                        <span id="hint_PROFILE[SETUP][URL_DATA_FILE]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][URL_DATA_FILE]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_FILENAME_HELP" )?>' );</script>
                        <?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_FILENAME" )?>
                    </td>
                ...
                <tr id="tr_type_file">
                    <?if( ( $arProfile["TYPE"] != "advantshop" ) ){?>
                        <td width="40%" class="adm-detail-content-cell-l">
                            <span id="hint_PROFILE[SETUP][FILE_TYPE]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][FILE_TYPE]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_FILE_TYPE_HELP" )?>' );</script>
                            <label for="PROFILE[SETUP][FILE_TYPE]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_FILE_TYPE" )?></label>
                        </td>
                        <td width="60%" class="adm-detail-content-cell-r">
                            ...
                            foreach( $profileUtils->GetFileExportType() as $type ){
                                ...
                                <input type="radio" name="PROFILE[SETUP][FILE_TYPE]" value="<?=$type?>" <?=$checked?> onchange="ChangeFileType(this.value)"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_FILE_".strtoupper( $type ) )?>
                            <?}?>
                        </td>
                    <?}
                    else{?>
                ...
                <tr id="tr_type_run">
                    <td width="40%" class="adm-detail-content-cell-l">
                        <span id="hint_PROFILE[SETUP][TYPE_RUN]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][TYPE_RUN]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_TYPE_HELP" )?>' );</script>
                        <label for="PROFILE[SETUP][TYPE_RUN]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_TYPE" )?></label>
                    </td>
                    <td width="60%" class="adm-detail-content-cell-r">
                        ...
                        foreach( $profileUtils->GetRunType() as $type ){
                            ...
                            <input type="radio" name="PROFILE[SETUP][TYPE_RUN]" value="<?=$type?>" <?=$checked?> onchange="ChangeRunType(this.value)"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_".strtoupper( $type ) )?>
                        <?}?>
                    </td>
                </tr>
                ...
                <tr id="tr_run_new_window" class="<?=$hideRunNewWindow?>">
                    ...
                    <td width="60%" class="adm-detail-content-cell-r" align="left">
                        <a class="adm-btn <?if( $exportTimeStamp > $profileTimeStamp ):?>adm-btn-save<?else:?>adm-btn-red<?endif;?>" href="/bitrix/tools/kit.exportpro/kit_exportpro.php?ID=<?=$ID?>" target="_blank"><?if( !empty( $arProfile["SETUP"]["URL_DATA_FILE"] ) ):?><?=GetMessage( "KIT_EXPORTPRO_RERUN_FILE_EXPORT" )?><?else:?><?=GetMessage( "KIT_EXPORTPRO_RUN_FILE_EXPORT" )?><?endif;?></a>
                        <?if( !$exportTimeStamp || ( $exportTimeStamp < $profileTimeStamp ) ){?>
                            <br/><br/>
                            <span class="important-info"><?if( !$exportTimeStamp ):?><?=GetMessage( "KIT_EXPORTPRO_FILE_EXPORT_NOT_EXIST" )?><?elseif( $exportTimeStamp < $profileTimeStamp ):?><?=GetMessage( "KIT_EXPORTPRO_FILE_EXPORT_NEED_RERUN" )?><?endif;?></span>
                        <?}?>
                    </td>
                </tr>
                <tr id="tr_run_new_window_cron" class="<?=$hideCronInfo?>">
                    <td width="40%" class="adm-detail-content-cell-l"></td>
                    <td width="60%" class="adm-detail-content-cell-r" align="left">
                        <a class="adm-btn <?if( $exportTimeStamp > $profileTimeStamp ):?>adm-btn-save<?else:?>adm-btn-red<?endif;?>" onclick="$( 'input[name=apply]' ).click();"><?if( !empty( $arProfile["SETUP"]["URL_DATA_FILE"] ) ):?><?=GetMessage( "KIT_EXPORTPRO_RERUN_FILE_EXPORT" )?><?else:?><?=GetMessage( "KIT_EXPORTPRO_RUN_FILE_EXPORT" )?><?endif;?></a>
                        <?if( !$exportTimeStamp || ( $exportTimeStamp < $profileTimeStamp ) ):?>
                            <br/><br/>
                            <span class="important-info"><?if( !$exportTimeStamp ):?><?=GetMessage( "KIT_EXPORTPRO_FILE_EXPORT_NOT_EXIST" )?><?elseif( $exportTimeStamp < $profileTimeStamp ):?><?=GetMessage( "KIT_EXPORTPRO_FILE_EXPORT_NEED_RERUN" )?><?endif;?></span>
                        <?endif;?>
                ...
                <?if( !empty( $arProfile["SETUP"]["URL_DATA_FILE"] ) ):?>
                    ...
                    <tr id="tr_open_new_window">
                        <td width="40%" class="adm-detail-content-cell-l"></td>
                        <td width="60%" class="adm-detail-content-cell-r" align="left">
                            <span id="hint_OPEN_INNEW_WINDOW"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_OPEN_INNEW_WINDOW' ), '<?=GetMessage( "KIT_EXPORTPRO_OPEN_INNEW_WINDOW_HELP" )?>' );</script>
                            
                            <a href="<?=$urlDataFile?>" target="_blank"><?=GetMessage( "KIT_EXPORTPRO_OPEN_INNEW_WINDOW" )?></a>
                        </td>
                    </tr>
                <?endif?>
                ...
                <tr id="tr_cron_info" class="<?=$hideCronInfo?>">
                    <td colspan="2">
                        <?=BeginNote();?>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES0" );?><br><br>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES1" );?><br><br>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES3" );?>
                        <b><?=$_SERVER["DOCUMENT_ROOT"];?>/bitrix/crontab/crontab.cfg</b>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES4" );?><br><br>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES5" );?><br>
                        <b>crontab <?=$_SERVER["DOCUMENT_ROOT"];?>/bitrix/crontab/crontab.cfg</b><br>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES6" );?><br>
                        <b>crontab -l</b><br>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES7" );?><br>
                        <b>crontab -r</b><br><br>
                        
                        <?$arRetval = array();
                        @exec( "crontab -l", $arRetval );
                        if( is_array( $arRetval ) && !empty( $arRetval ) ){?>
                            <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES8" );?><br>
                        ...
                        <?}?>
                        <?=GetMessage( "KIT_EXPORTPRO_CES_NOTES10" );?>
                    </td>
                </tr>
                <tr id="tr_date_start" class="<?=$hideDateStart?>">
                    <td width="40%" class="adm-detail-content-cell-l" style="vertical-align:middle">
                        <span id="hint_PROFILE[SETUP][DAT_START]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][DAT_START]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_DATESTART_HELP" )?>' );</script>
                        <label for="PROFILE[SETUP][DAT_START]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_DATESTART" )?></label><br>
                    </td>
                    <td width="60%" class="adm-detail-content-cell-r">
                        <div class="adm-input-wrap adm-input-wrap-calendar">
                            ...
                            <span class="adm-calendar-icon" title="<?=GetMessage( "KIT_EXPORTPRO_NAJMITE_DLA_VYBORA_D" )?>" onclick="BX.calendar({node:this, field:'PROFILE[SETUP][DAT_START]', form: '', bTime: true, bHideTime: false});"></span>
                        </div>
                    </td>
                </tr>
                <tr id="tr_date_period" class="<?=$hideDatePeriod?>">
                    <td width="40%" class="adm-detail-content-cell-l">
                        <span id="hint_PROFILE[SETUP][PERIOD]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][PERIOD]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_PERIOD_HELP" )?>' );</script>
                        <label for="PROFILE[SETUP][PERIOD]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_PERIOD" )?></label>
                    </td>
                    <td width="60%" class="adm-detail-content-cell-r">
                ...
                <tr id="tr_cron_threads" class="<?=$hideCronThread?>">
                    <td width="40%" class="adm-detail-content-cell-l">
                        <span id="hint_PROFILE[SETUP][THREADS]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][THREADS]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_THREADS_HELP" )?>' );</script>
                        <label for="PROFILE[SETUP][THREADS]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_THREADS" )?></label>
                    </td>
                    <td width="60%" class="adm-detail-content-cell-r">
                ...
                <tr id="tr_cron_is_period" class="<?=$hideCronIsPeriod?>">
                    <td width="40%" class="adm-detail-content-cell-l">
                        <span id="hint_PROFILE[SETUP][IS_PERIOD]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][IS_PERIOD]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_IS_PERIOD_HELP" )?>' );</script>
                        <label for="PROFILE[SETUP][IS_PERIOD]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_IS_PERIOD" )?></label>
                    </td>
                    <td width="60%" class="adm-detail-content-cell-r">
...
<tr>
    <td width="40%" class="adm-detail-content-cell-l">
        <span id="hint_PROFILE[SETUP][LAST_START_EXPORT]"></span><script type="text/javascript">BX.hint_replace( BX( 'hint_PROFILE[SETUP][LAST_START_EXPORT]' ), '<?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_LSATSTART_HELP" )?>' );</script>
        <label for="PROFILE[SETUP][LAST_START_EXPORT]"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_LSATSTART" )?></label>
    </td>
    <td width="60%" class="adm-detail-content-cell-r">
...
<?if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/kit.exportpro/export_{$arProfile["ID"]}_run.lock" ) ):?>
    <tr id="unlock-container">
        <td>
            <?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_EXPORT_RUN" );?>
        </td>
        <td>
            <a class="adm-btn adm-btn-save" onclick="UnlockExport( <?=$arProfile["ID"]?> )"><?=getMessage( "KIT_EXPORTPRO_RUNTYPE_UNLOCK" )?></a>
        </td>
    </tr>
<?endif?>
<tr>
    <td align="center" colspan="2">
        <div class="adm-info-message"><?=GetMessage( "KIT_EXPORTPRO_RUNTYPE_INFO" )?></div>
    </td>
</tr>
