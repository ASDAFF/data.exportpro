...
class CExportproAgent{
    public static function StartExport( $profileId ){
        ...
        $export = new CDataExportproExport( intval( $profileId ) );
        $export->Export( "agent" );
    ...
    public static function AddAgent( $profileID, $setup ){
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/crontab/crontab.cfg" ) ){
            ...
            $cfgData = preg_replace( "#.*bitrix\/modules\/data.exportpro\/tools\/cron_events.php(\r)*\n#i", "", $cfgData);
            $cfgData = preg_replace( "#.*bitrix\/modules\/data.exportpro\/tools\/cron_events.php#i", "", $cfgData);
            $cronTask = "* * * * * php -f {$_SERVER["DOCUMENT_ROOT"]}/bitrix/modules/data.exportpro/tools/cron_events.php";
...
class CExportproCron{
    ...
    public static function CronRun( $profileID, $setup, $delete_cron = false ){
		...
		foreach( $cron_list as $id => $cronRecord ){
            if( strpos( $cronRecord, "/data.exportpro/tools/cronrun.php $profileID" ) ){
                unset( $cron_list[$id] );
            }
        }
        ...
        if( !$delete_cron ){
			foreach( $cronTime as $strTime ){
				$cron_list[] = "$strTime php -f {$_SERVER["DOCUMENT_ROOT"]}/bitrix/modules/data.exportpro/tools/cronrun.php $profileID \"{$_SERVER["DOCUMENT_ROOT"]}\"";
			}
		}
